...
public class Post {
    ...
    @Column(nullable = false, columnDefinition = "TEXT") //The variable can take up large content; TEXT
    ...
    @ManyToOne(fetch = FetchType.LAZY) //optimises performance by loading post authors only when they're explicitly accessed
    @JoinColumn(name = "author_id", nullable = false) //Specifies that the column author-id will reference author-id
    private User author;
